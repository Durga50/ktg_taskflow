...
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.2/css/all.min.css">
    ...
    <div class="col-md-4 pt-2 pb-2 d-flex justify-content-center align-items-center">
        <button type="submit" id="designationBtn" class="btn" style="background: rgb(0, 148, 255); border-radius: 25px; color: white; width: 190px;" disabled>
...
<script src="js/demo/datatables-demo.js"></script>
  ...
  <script>
    ...
    // 🚫 Only allow form submission if editId is set (i.e., Update mode)
    if (!editId) return;
    ...
    $.ajax({
        ...
        success: function (response) {
            Swal.fire({
                title: "Updated!",
                text: "Designation Successfully Edited",
                icon: "success",
                confirmButtonColor: "rgb(0, 148, 255)"
            }).then(() => {
                $("#designationName").val("");
                $("#designationBtn")
                    .html('<i class="fas fa-briefcase"></i>&nbsp; Add Designation')
                    .prop("disabled", true); // 🔒 Disable button after update

                editId = null;
                fetchdesignation(); // 🔁 Refresh table
            });
        },
        error: function () {
            Swal.fire({
